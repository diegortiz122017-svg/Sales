...
const getInventory = async ({ type, make, model, maxPrice, minYear, maxMileage, drivetrain, bodyStyle, page = 1, limit = 12 } = {}) => {
  ...
  if (minYear    && !isNaN(parseInt(minYear)))    { conditions.push('year >= ?');    params.push(parseInt(minYear)); }
  if (maxMileage && !isNaN(parseInt(maxMileage))) { conditions.push('mileage <= ?'); params.push(parseInt(maxMileage)); }
